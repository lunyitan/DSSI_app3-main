...
# load the model.pkl
path = r'C:\Users\Tanlu\Desktop\School\DSI\Day2\DSSI_app3-main\model.pkl'
...
def prediction(int_rate, emp_length, annual_inc, delinq_2yrs, fico_range_high, revol_bal, open_acc):
    ...
    prediction = model.predict(
        [[int_rate, emp_length, annual_inc, delinq_2yrs, fico_range_high, revol_bal, open_acc]])
    if prediction == 0:
        pred = 'Rejected'
    else:
        pred = 'Approved'
...
# display the front end aspect
    ...
    if st.button("Predict"):
        assessment = prediction(int_rate, emp_length, annual_inc,
                                delinq_2yrs, fico_range_high, revol_bal, open_acc)
    ...
    # st.balloons()
    # other tags include st.error, st.warning, st.help etc.
